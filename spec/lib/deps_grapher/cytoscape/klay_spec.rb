...
RSpec.describe DepsGrapher::Cytoscape::Klay do
  ...
  before do
    DepsGrapher::Layer.new do
      ...
      source do
        ...
        glob_pattern File.join("cytoscape", "klay.rb")
      end
      color do
        background "blue"
        border "black"
    ...
    allow(downloader).to receive(:download).with("https://unpkg.com/klayjs/klay.js")
    allow(downloader).to receive(:download).with("https://raw.githubusercontent.com/cytoscape/cytoscape.js-klay/master/cytoscape-klay.js")
  end

  it "command_option is `cy:klay`" do
    expect(DepsGrapher::Visualizer::Registry.send(:registry)["cy:klay"].first).to eq described_class
  end
  ...
  describe "#render" do
    ...
    it "renders the graph" do
      ...
      expect(html).to include('<script src="klay.js"></script>')
      expect(html).to include('<script src="cytoscape-klay.js"></script>')
      expect(html).to include('"name":"klay"')
