...
RSpec.describe DepsGrapher::Cytoscape::Fcose do
  ...
  before do
    DepsGrapher::Layer.new do
      ...
      source do
        ...
        glob_pattern File.join("cytoscape", "fcose.rb")
      end
      color do
        background "blue"
        border "black"
    ...
    allow(downloader).to receive(:download).with("https://unpkg.com/cytoscape-fcose/cytoscape-fcose.js")
  end

  it "command_option is `cy:fcose`" do
    expect(DepsGrapher::Visualizer::Registry.send(:registry)["cy:fcose"].first).to eq described_class
  end
  ...
  describe "#render" do
    ...
    it "renders the graph" do
      ...
      expect(html).to include('<script src="cytoscape-fcose.js"></script>')
      expect(html).to include('"name":"fcose"')
